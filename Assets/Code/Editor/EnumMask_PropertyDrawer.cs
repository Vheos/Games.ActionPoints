        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            if (foldoutOpen)
            {
                ...
                if (layout == EnumMaskLayout.Vertical)
                    return EditorGUIUtility.singleLineHeight * (Enum.GetValues(fieldInfo.FieldType).Length + 1);
                else
            ...
            if (foldoutOpen)
            {
                ...
                */
                ...
                if (layout == EnumMaskLayout.Vertical)
                {
                    ...
                    {
                        if (EditorGUI.Toggle(new Rect(position.x+20, position.y + EditorGUIUtility.singleLineHeight * (1 + i), position.width, EditorGUIUtility.singleLineHeight), Enum.GetNames(fieldInfo.FieldType)[i], IsSet(i)))
                            ToggleIndex(i, true);
                        else
                            ToggleIndex(i, false);
                    }
